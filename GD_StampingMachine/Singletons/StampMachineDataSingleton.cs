...
namespace GD_StampingMachine.Singletons
{
  ...
  new()
                            ...
                            do
                            {
                                ...
                                try
                                {
                                    ...
                                    if (firstConnected)
                                    {
                                        ...
                                        //var ret7 = await opcUaClient.GetLubricationSettingTime();
                                        //var ret8 = await opcUaClient.GetLubricationSettingOnTime();
                                        //var ret9 = await opcUaClient.GetLubricationSettingOffTime();
                                        ...
                                        var engravingFeedingfeedSVelocity = await GetEngravingFeedingSetupVelocityAsync();
                                        //初始化後直接設定其他數值
    ...
    (sender, e) => this.StoppedLamp = e.NewValue);
                                     ...
                                     /*   await this.opcUaClient.SubscribeNodeDataChangeAsync<int>(StampingOpcUANode.OpcMonitor1.sv_OpcAlarmCount,
                                          (sender, e) =>
                                          {
                                              if (e.NewValue > 0)
                                              {
                                            ...
                                            if (result)
                                            {
                                                ...
                                                if (collection != null)
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        {
                                                            int index = i;//防止閉包問題
                                                            if (MachineSettingBaseCollection.TryGetValue(index, out var machineSetting)
                                                            && collection.TryGetValue(index, out var plateMonitorVM))
                                                            {
                                                                ...
                                                                || machineSetting.ShearingIsFinish != plateMonitorVM.ShearingIsFinish)
                                                                {
                                                                    ...
                                                                    Application.Current?.Dispatcher.Invoke(() =>
                                                                    {
                                                                        try
                                                                        {
                                                                            if (MachineSettingBaseCollection != null)
                                                                                MachineSettingBaseCollection[index] = plateMonitorVM;
                                                                        }
                                                                        catch
                                                                        {
...
*/
     ...
     /*   public async Task<bool> SetHMIIronPlateName(StampingOpcUANode.sIronPlate ironPlateType, string StringLine)
       ...
       /* public Task SubscribeDataMatrixTCPIP(Action<string> action)
        ...
        public Task<bool> ReadNodesAsync(IEnumerable<(Action<object>, string)> NodeTrees)
        {
        ...
        public Task<(bool, IEnumerable<object>)> ReadNodesAsync(IEnumerable<string> nodeIds)
        {
            return Task.Run(async () =>
            {
                if (opcUaClient != null)
        ...
        public Task<(bool result, T? values)> ReadNodeAsync<T>(string NodeTree)
        {
            return Task.Run(async () =>
            {
                if (opcUaClient != null)
        ...
        public Task<IEnumerable<bool>> WriteNodesAsync<T>(Dictionary<string, object> NodeTrees)
        {
            return Task.Run(async () =>
            {
                for (int i = 0; i < 5; i++)
        ...
        public Task<bool> WriteNodeAsync<T>(string NodeTreeString, T WriteValue)
        {
            return Task.Run(async() =>
            {
                 for (int i = 0; i < 5; i++)
