  ...
  new IO_InfoModel()
        ...
        public async Task StartScanOpcuaAsync()
        {
            ...
            if (scanTask == null || scanTask.Status != TaskStatus.Running)
            {
                ...
                try
                {
                    ...
                    scanTask = RunScanTaskAsync(token);
                }
                catch (Exception ex)
                {
                        ...
                        /*  _ = Task.Run(async () =>
                           {
                               try
                               {
                                   ...
                                   {
                                       var oldState = GD_OpcUaClient.State;
                                ...
                                while (true)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        if (GD_OpcUaClient.IsConnected)
                                        {
                                            ...
                                            var BlockingCylindere_IsDown = GetBlockingCylinder_Position_isDownAsync();
        ...
        public AsyncRelayCommand<object> FeedingXAxisFwdCommand
        {
            get => new(async obj =>
            {
                    ...
                    {
                        await SetFeedingXAxisFwdAsync(isActived);
                    }
            }, obj => IsConnected);
        }
        ...
        public AsyncRelayCommand<object> FeedingXAxisBwdCommand
        {
            get => new(async obj =>
            {
                if (obj is bool isActived)
                {
                    await SetFeedingXAxisBwdAsync(isActived);
                }
            });
        }
        ...
        public AsyncRelayCommand ActiveHydraulicPumpMotor
        {
            get => _activeHydraulicPumpMotor??= new AsyncRelayCommand(async () =>
            {
                if (GD_OpcUaClient.IsConnected)
                {
                    ...
                    if (isActived.Item1)
                    {
                        await SetHydraulicPumpMotorAsync(!isActived.Item2);
                    }
                }
        ...
        public AsyncRelayCommand<object> EngravingYAxisFwdCommand
        {
            get => _engravingYAxisFwdCommand ??= new(async obj =>
            {
                if (obj is bool isActived)
                {
                    ...
                    await SetEngravingYAxisFwdAsync(isActived);
                }
            }, obj => IsConnected);
        }
        ...
        public AsyncRelayCommand<object> EngravingYAxisBwdCommand
        {
            get => _engravingYAxisBwdCommand ??= new(async obj =>
            {
                if (obj is bool isActived)
                {
                    await SetEngravingYAxisBwdAsync(isActived);
                }
            });
        }
        ...
        public AsyncRelayCommand<object> EngravingRotateClockwiseCommand
        {
            get => _engravingRotateClockwiseCommand??= new AsyncRelayCommand<object>(async obj =>
            {
                if (obj is bool isActived)
                {
                    ...
                    if (!isActived && (await GetOperationModeAsync()).Item2 == OperationModeEnum.Manual)
                        await Task.Delay(200);
                    await SetEngravingRotateCWAsync(isActived);
                }
            });
        }
         ...
         public AsyncRelayCommand<object> EngravingRotateCounterClockwiseCommand
        {
            get => _engravingRotateCounterClockwiseCommand ??= new AsyncRelayCommand<object>(async obj =>
            {
                if (obj is bool isActived)
                {
                    if (!isActived && (await GetOperationModeAsync()).Item2 == OperationModeEnum.Manual)
                        await Task.Delay(200);
                    await SetEngravingRotateCCWAsync(isActived);
                }
            });
        }
...
*/
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<(bool, bool)> GetHydraulicCutting_Position_CutPointAsync()
        {
     ...
     /*   public async Task<bool> SetHMIIronPlateName(StampingOpcUANode.sIronPlate ironPlateType, string StringLine)
        ...
        public async Task<bool> SetHMIIronPlateAsync(IronPlateDataModel ironPlateData)
        {
            ...
            var ret = await GD_OpcUaClient.WriteNodesAsync(wNodeTrees);
            //設定完後須更新hmi
        ...
        public async Task<bool> SetIronPlateDataCollectionAsync(List<IronPlateDataModel> ironPlateDataList)
        {
            ...
            for (int i = 0; i < write_ironPlateDataList.Count; i++)
            {
                ...
                for (int j = 0; j < 5; j++)
                {
                    var WriteBooleanList = await GD_OpcUaClient.WriteNodesAsync(wNodeTreeNodes);
            ...
            // return true;
            //  return await GD_OpcUaClient.WriteNodesAsync(wNodeTrees);
        }
